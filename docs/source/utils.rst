...
| BIGREST has a Python module called utils that has multiple usefull functions.
| Use these functions to help you working with iControl REST API.
...
| If you just working with BIGREST, either BIGIP or BIGIQ classes, it can use token internally if you enable token use.
...
| If you just working with BIGREST, BIGIQ class only, it can use refresh token internally if you provide a refresh token.
